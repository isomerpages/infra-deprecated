...
resource "github_repository_deploy_key" "isomer_backup_github_repository_deploy_key_tech_gov_sg" {
  title      = "Netlify"
  repository = "isomerpages-govtech"
  key        = "${netlify_deploy_key.isomer_backup_netlify_deploy_key_tech_gov_sg.public_key}"
  read_only  = false
}

// Create a webhook that triggers Netlify builds on push.
resource "github_repository_webhook" "isomer_backup_github_repository_webhook_tech_gov_sg" {
  repository = "isomerpages-govtech"
  name       = "web"
  ...
  depends_on = ["netlify_site.isomer_backup_netlify_site_tech_gov_sg"]
}

resource "netlify_deploy_key" "isomer_backup_netlify_deploy_key_tech_gov_sg" {}
...
resource "netlify_site" "isomer_backup_netlify_site_tech_gov_sg" {
  ...
  repo {
    ...
    deploy_key_id = "${netlify_deploy_key.isomer_backup_netlify_deploy_key_tech_gov_sg.id}"
    dir           = "_site/"
    provider      = "github"
